  ...
  if (details) {
    ...
    if (testResult && Number(testResult.numTestsRun) > 0) {
      if (verbose) {
        ...
        if (codeCoverages.length > 0) {
          var totalNotCovered = _.sum(_.map(codeCoverages, function(c) { return +c.numLocationsNotCovered; }));
          var totalLocations = _.sum(_.map(codeCoverages, function(c) { return +c.numLocations; }));
          var totalCovered = totalLocations - totalNotCovered;
          var percent = Math.floor(100.0 * (totalCovered / totalLocations));
          logger.log('');
          logger.log('Total Coverage: ' + percent + '%');
          logger.log('');
        }
      }
      ...
      if (verbose) {
        ...
        if (testSuccesses.length > 0) {
          ...
          if (verbose == 'condenseSuccesses') logger.log(' - ' + testSuccesses.length + ' tests passed');
        }
        if (verbose != 'condenseSuccesses') {
      ...
      testFailures.forEach(function(f) {
        ...
        if (f.stackTrace && _.isString(f.stackTrace)) {
    ...
    if (verbose) {
      ...
      if (successes.length > 0) {
        logger.log('Deployment Successes:');
      }
      if (verbose == 'condenseSuccesses') {
        var totalChanged = _.sum(_.map(successes, function(s) { return s.changed === 'true' ? 1 : 0; }));
        var totalCreated = _.sum(_.map(successes, function(s) { return s.created === 'true' ? 1 : 0; }));
        var totalDeleted = _.sum(_.map(successes, function(s) { return s.deleted === 'true' ? 1 : 0; }));
        var totalUnmodified = successes.length - totalChanged - totalCreated - totalDeleted;
        logger.log(' - ' + totalChanged + ' changed');
        logger.log(' - ' + totalCreated + ' created');
        logger.log(' - ' + totalDeleted + ' deleted');
        logger.log(' - ' + totalUnmodified + ' unmodified');
      } else {
    ...
    if (failures) {
      if (failures.length > 0) {
        logger.log('Deployment Failures:');
      }
